...
function Signup() {
  ...
  const signup = (event) => {
    ...
    axios.post(`${API_BASE_URL}/users`, reqData, config)
      .then((response) => {
        alertFunction(' Regestration Successfully, Please proceed to  Login ! ', 'success')
         setEmail("");
         setFname("");
         setLname("");
         setPhone("");
      ...
      <div className='text-muted shadow-sm rounded p-3 lh-2'>
        <form onSubmit={(ev) => signup(ev)}>
          ...
          <div className='d-grid'>
            <button type="submit" onChange={(e) => signup(e.target.value)} className="btn btn-primary">Signup</button>
          </div>
